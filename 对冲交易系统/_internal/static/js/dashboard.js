...
function updateExecutionMetrics(metrics) {
    ...
    // 格式化百分比值（总收益率、年化收益率、最大回撤、胜率）
    document.getElementById('execution-total-return').textContent = 
        typeof metrics.total_return === 'number' ? 
        (metrics.total_return * 100).toFixed(2) + '%' : 
        parseFloat(metrics.total_return).toFixed(2) + '%';
    
    document.getElementById('execution-annual-return').textContent = 
        typeof metrics.annual_return === 'number' ? 
        (metrics.annual_return * 100).toFixed(2) + '%' : 
        parseFloat(metrics.annual_return).toFixed(2) + '%';
    
    document.getElementById('execution-sharpe-ratio').textContent = 
        typeof metrics.sharpe_ratio === 'number' ? 
        metrics.sharpe_ratio.toFixed(2) : 
        parseFloat(metrics.sharpe_ratio).toFixed(2);
    
    document.getElementById('execution-max-drawdown').textContent = 
        typeof metrics.max_drawdown === 'number' ? 
        (metrics.max_drawdown * 100).toFixed(2) + '%' : 
        parseFloat(metrics.max_drawdown).toFixed(2) + '%';
    
    document.getElementById('execution-win-rate').textContent = 
        typeof metrics.win_rate === 'number' ? 
        (metrics.win_rate * 100).toFixed(2) + '%' : 
        parseFloat(metrics.win_rate).toFixed(2) + '%';
    
    document.getElementById('execution-profit-loss-ratio').textContent = 
        typeof metrics.profit_loss_ratio === 'number' ? 
        metrics.profit_loss_ratio.toFixed(2) : 
        parseFloat(metrics.profit_loss_ratio).toFixed(2);
    
    document.getElementById('execution-total-trades').textContent = 
