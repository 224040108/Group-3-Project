...
def start_execution():
    """启动执行系统"""
    if not execution_system.running:
        execution_system.start()
        return jsonify({'status': 'success', 'message': '执行系统已启动'})
    else:
        return jsonify({'status': 'error', 'message': '执行系统已经在运行'})

@app.route('/api/execution/stop', methods=['POST'])
def stop_execution():
    """停止执行系统"""
    if execution_system.running:
        execution_system.stop()
        return jsonify({'status': 'success', 'message': '执行系统已停止'})
    else:
        return jsonify({'status': 'error', 'message': '执行系统未在运行'})

@app.route('/api/execution/status', methods=['GET'])
def get_execution_status():
    """获取执行系统状态"""
    return jsonify({
        'running': execution_system.running,
        'last_update': execution_system.last_update_time.strftime('%Y-%m-%d %H:%M:%S') if execution_system.last_update_time else None
    })
